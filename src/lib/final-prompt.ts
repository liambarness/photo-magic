      ...
      if (modelProfile.kind === "human") {
        ...
        if (usesVisibleFace) {
          if (humanProfileHasFaceReferences(modelProfile)) {
            parts.push(
              "Use the attached face reference images as the authoritative source for the model's facial identity, facial features, skin tone, and natural expression. If the uploaded product/source image already contains a different person or visible face, replace that person's facial identity with the selected human model's face from the references. Use the uploaded product/source image as the authoritative source for the garment/product, pose, body framing, visible body extent, subject scale, canvas composition, product fit, logo placement, artwork, colors, camera angle, and background. Preserve the source crop exactly: do not zoom out, reframe, convert an upper-body crop into a full-body shot, or add legs, feet, shoes, hands, or body areas that are not visible in the source image. Do not preserve the source person's original face. Do not copy clothing, background, pose, lighting, or camera angle from the face reference images."
            );
          } else {
            parts.push(
